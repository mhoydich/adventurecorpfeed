...
//swipe up &down get triggered also with mouse 
	...
	mc.on('pinchout', function(ev){
		...
		/*console.log(ev.type)
		childs[choose].scale.x += 0.003;
		childs[choose].scale.y += 0.003;
		childs[choose].scale.z += 0.003;*/
...
cancelAnimationFrame(myReq);
	...
	loader.load(model2, function ( collada2 ) { 
		...
		if (anim_model.skins.length>0){
			...
			console.log(skin2)
			animation = new THREE.Animation(childs[index-1], skin2.geometry.animation);
  ...
  $("#loading").show();
	...
	loader.load(model, function ( collada ) { 
		...
		if (bots[botIndex].skins.length>0){
			...
			//childs[index].castShadow= true; doesn't work
			animation = new THREE.Animation(botChilds[botIndex], botChilds[botIndex].geometry.animation);
